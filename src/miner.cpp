...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool checkApproved)
{
    ...
    int ser_flags = (nHeight < params.BTHHeight) ? SERIALIZE_BLOCK_LEGACY : 0;
    ...
    if (!TestBlockValidity(state, chainparams, *pblock, pindexPrev, false, false, checkApproved)) {
